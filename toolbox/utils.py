...
def class_to_RGB(label, N, cmap=None, normalized=False):
    ...
    for i_class in range(N):
        # print(label_color[label_class == i_class])
...
def save_ckpt(logdir, model,kind, cur_epoch=1):
    if kind=='end':
        ...
        state = model.module.state_dict() if hasattr(model, 'module') else model.state_dict()
        torch.save(state, os.path.join(logdir, 'modelA_best.pth'))
    elif kind=='end_A':
        state = model.module.state_dict() if hasattr(model, 'module') else model.state_dict()
        torch.save(state, os.path.join(logdir, 'modelA_end.pth'))
    elif kind=='best_B':
        state = model.module.state_dict() if hasattr(model, 'module') else model.state_dict()
        torch.save(state, os.path.join(logdir, 'modelB_best.pth'))
    elif kind=='end_B':
        state = model.module.state_dict() if hasattr(model, 'module') else model.state_dict()
        torch.save(state, os.path.join(logdir, 'modelB_end.pth'))
    elif kind == 'epoch':
        state = model.module.state_dict() if hasattr(model, 'module') else model.state_dict()
...
def load_ckpt(logdir, model,kind):
    if kind=='end':
        ...
        model.load_state_dict(torch.load(save_pth))
    if kind=='end_A':
        save_pth = os.path.join(logdir, 'modelA_end.pth')
        model.load_state_dict(torch.load(save_pth))
    elif kind=='best_A':
        save_pth = os.path.join(logdir, 'modelA_best.pth')
        model.load_state_dict(torch.load(save_pth))
    if kind=='end_B':
        save_pth = os.path.join(logdir, 'modelB_end.pth')
        model.load_state_dict(torch.load(save_pth))
    elif kind=='best_B':
        save_pth = os.path.join(logdir, 'modelB_best.pth')
        model.load_state_dict(torch.load(save_pth))
    elif kind=='epoch':
        save_pth = os.path.join(logdir, 'model_129.pth')
        model.load_state_dict(torch.load(save_pth))
    return model
